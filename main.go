...
func main() {
	...
	if (time.Now().Unix() - healthz.LastHeartbeat/1000) < 1800 {
		fmt.Println("Activity in the last 30 minutes, timestamp: ", healthz.LastHeartbeat, "difference from now: ", time.Now().Unix()-healthz.LastHeartbeat/1000)
		os.Exit(0)
	}
